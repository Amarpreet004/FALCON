...
const getContainer = () => {
  const id = "_globalCoolMode_effect";
...
const applyGlobalParticleEffect = (options?: CoolParticleOptions): (() => void) => {
  instanceCounter++;
  ...
  const tapHandler = (e: MouseEvent | TouchEvent) => {
    ...
    // Generate multiple particles for a more dramatic effect
    ...
    for (let i = 0; i < particleCount; i++) {
      ...
      }, i * 20); // Stagger particle generation
    }
  };
...
interface GlobalCoolModeProps {
  options?: CoolParticleOptions;
  children: React.ReactNode;
}

export const GlobalCoolMode: React.FC<GlobalCoolModeProps> = ({ 
  children, 
  options 
}) => {
  ...
  useEffect(() => {
    cleanupRef.current = applyGlobalParticleEffect(options);
